...
def merkle_assignment():
    ...
    random_leaf_index = random.randint(1, num_of_primes -1) # updated generate a random index from primes to claim (0 is already claimed)
    ...
    if sign_challenge_verify(challenge, addr, sig):
        ...
        print(f"Transaction hash placeholder: {tx_hash}")
...
def generate_primes(num_primes):
    ...
    if num_primes <= 6:
        ...
        import math
        limit = int(num_primes * (math.log(num_primes) + math.log(math.log(num_primes)))) + 100
...
def send_signed_msg(proof, random_leaf):
    ...
    fn = None
    try:
        fn = contract.functions.submit(random_leaf, proof)
    except Exception:
        try:
            fn = contract.functions.submit(proof, random_leaf)
        except Exception as e:
            raise RuntimeError(f"submit() function signature not recognized: {e}")
    ...
    # Estimate gas if possible; fall back to a safe buffer
    try:
        gas_est = fn.estimate_gas({'from': acct.address})
        tx_data['gas'] = int(gas_est * 1.2)  # 20% buffer
    except Exception:
        tx_data['gas'] = 500_000

    tx = fn.build_transaction(tx_data)
